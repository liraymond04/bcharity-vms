...
const VolunteerHome: React.FC = () => {
  ...
  const getSlug = (key: string) => {
    const item =
      ...
      currentUser.attributes.filter((item) => item.key === key).at(0)
    if (item) {
      return item.value
    }
    return ''
            ...
            <div className="flex flex-row">
              <div className="flex flex-col items-center space-y-5 align-middle w-3/12 ml-8 mt-10">
                ...
                <div className="flex">
                  ...
                  <div className="ml-1">{currentUser.stats.totalFollowers}</div>
                </div>

                <div className="flex">
                  <h1 suppressHydrationWarning>{t('following')}</h1>
                  <div className="ml-1">{currentUser.stats.totalFollowing}</div>
                </div>
